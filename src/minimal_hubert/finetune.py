...
def finetune(name: str, workdir: Path, checkpoint: Path, manifest: str) -> None:
    max_steps, seed = 20_000, 0
    with ExitStack() as stack:
        set_seed(seed)
        setup_pytorch(use_deterministic=False)
        setup_environment()
        rundir = workdir / "discophon-hubert-ft" / name
        ...
        loader = build_dataloader_with_labels(hubert_ft_data_config(manifest), MaskingConfig())
...
if __name__ == "__main__":
    ...
    finetune(args.name, args.workdir, args.checkpoint, args.manifest)
