...
from minimal_hubert.compatibility import export_state_dict_to_hf
...
@given(waveforms=waveforms())
@hypothesis_settings
def test_export_state_dict_to_hf(
    my_hubert: HuBERT,
    hf_hubert: HubertModel,
    device: torch.device,
    waveforms: torch.Tensor,
    ...
    waveforms = waveforms.to(device)
    hf_hubert_from_mine = deepcopy(hf_hubert)
    hf_hubert_from_mine.load_state_dict(export_state_dict_to_hf(my_hubert.state_dict()), strict=False)
    x = hf_hubert_from_mine(waveforms, output_hidden_states=True)
    y = my_hubert.get_intermediate_outputs(waveforms, before_residual=False)
    torch.testing.assert_close(x.last_hidden_state, y[-1])
    for xi, yi in zip(x.hidden_states[1:], y, strict=True):
        torch.testing.assert_close(xi, yi)
